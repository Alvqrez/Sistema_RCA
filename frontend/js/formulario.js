...
async function cargarAlumnos() {
  ...
  try {
    // Trae los alumnos inscritos en el grupo seleccionado
    const res = await fetch(`${BASE_URL_FORM}/api/alumnos`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const todos = await res.json();

    // Por ahora carga todos; cuando tengas la tabla inscripcion
    // cambia por: /api/grupos/${estado.grupoId}/alumnos
...
function pedirGuardarConfig() {
  if (!validarTodasPonderaciones()) {
    alert(
      "⚠️ Todas las unidades deben sumar exactamente 100% antes de guardar.",
    );
    return;
  }
