...
async function cargarMaterias() {
  ...
  materias.forEach(m => {
    const carrerasBadges = (m.carreras || []).map(c =>
...
function agregarCarreraLocal() {
  ...
  const id = sel.value;
  if (!id) { mostrarMensaje("Selecciona una carrera primero", "error"); return; }

  const yaEsta = [...carrerasActuales, ...carrerasPendientes].some(c => c.id_carrera === id);
  if (yaEsta) { mostrarMensaje("Esa carrera ya está vinculada", "error"); return; }
  ...
  carrerasPendientes.push({ id_carrera: id, nombre_carrera: carrera?.nombre_carrera || id, semestre });

  sel.value = ""; semInput.value = "1";
  renderChips();
}
...
function quitarCarrera(id_carrera) {
  ...
  carrerasActuales  = carrerasActuales.filter(c => c.id_carrera !== id_carrera);
  carrerasPendientes = carrerasPendientes.filter(c => c.id_carrera !== id_carrera);
  renderChips();
}
...
form.addEventListener("submit", async function(e) {
  ...
  const materia = {
    ...
    horas_teoricas:   parseInt(document.getElementById("horasTeoricas").value) || 0,
    horas_practicas:  parseInt(document.getElementById("horasPracticas").value)|| 0,
    no_unidades:      parseInt(document.getElementById("noUnidades").value)    || 0,
  };

  if (!materia.nombre_materia) { mostrarMensaje("El nombre de la materia es requerido.", "error"); return; }
  ...
  if (materiaEditando) {
    ...
    if (!materia.clave_materia) { mostrarMensaje("La clave es requerida.", "error"); return; }
  }
  ...
  try {
    ...
    if (!data.success) { mostrarMensaje(data.error || "Error al guardar.", "error"); return; }
    ...
    for (const c of carrerasPendientes) {
      await fetch(`${BASE_URL}/api/materias/${clave}/carreras`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ id_carrera: c.id_carrera, semestre: c.semestre, creditos: 0 })
      });
    }
...
async function editarMateria(clave) {
  const token = localStorage.getItem("token");
  try {
    const res  = await fetch(`${BASE_URL}/api/materias/${clave}`, { headers: { Authorization: `Bearer ${token}` } });
...
function cancelarEdicion() {
  ...
  document.getElementById("tituloFormMateria").textContent = "Registrar materia";
  const btnSubmit = document.querySelector("#formMateria [type='submit']");
...
async function eliminarMateria(clave) {
  if (!confirm(`¿Eliminar la materia "${clave}"? Esta acción no se puede deshacer.`)) return;
  const token = localStorage.getItem("token");
  try {
    const res  = await fetch(`${BASE_URL}/api/materias/${clave}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json();
...
function dragOverMaterias(e) { e.preventDefault(); document.getElementById("dropZoneMaterias").classList.add("drag-over"); }
function soltarCSVMaterias(e) {
  e.preventDefault();
  document.getElementById("dropZoneMaterias").classList.remove("drag-over");
  const file = e.dataTransfer.files[0]; if (file) procesarCSVMaterias(file);
}
function leerCSVMaterias(e) { const file = e.target.files[0]; if (file) procesarCSVMaterias(file); }

function procesarCSVMaterias(file) {
  const reader = new FileReader();
  reader.onload = (e) => {
    const lines = e.target.result.trim().split("\n").filter(Boolean);
    if (lines.length < 2) { document.getElementById("csvMateriasPreview").innerHTML = "<p style='color:var(--danger);font-size:.85rem;margin-top:8px'>Archivo vacío o sin datos.</p>"; return; }
...
function mostrarPreviewCSVMaterias(headers, data) {
  ...
  if (!data.length) { preview.innerHTML = "<p style='color:var(--danger);font-size:.85rem;margin-top:8px'>Sin datos válidos.</p>"; return; }
...
async function importarCSVMaterias() {
  ...
  try {
    const r    = await fetch(`${BASE_URL}/api/materias/csv`, { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`}, body:JSON.stringify({materias:csvMateriasData}) });
    const data = await r.json();
    if (!r.ok) throw new Error(data.error || "Error al importar");
    if (typeof toast === "function") { toast(`${data.insertados} materia(s) importadas correctamente`); if (data.errores?.length) toast(`${data.errores.length} fila(s) con errores`,"info"); }
    else alert(`${data.insertados} materia(s) importadas correctamente.`);
    if (data.errores?.length) console.table(data.errores);
    cerrarModalCSVMaterias(); cargarMaterias();
  } catch(err) { if (typeof toast==="function") toast(err.message,"error"); else alert(err.message); }
  finally { btn.disabled=false; btn.innerHTML=`<iconify-icon icon="lucide:upload"></iconify-icon> Importar`; }
}

async function exportarCSVMaterias() {
  const token = localStorage.getItem("token");
  try {
    const r       = await fetch(`${BASE_URL}/api/materias`, { headers:{Authorization:`Bearer ${token}`} });
    const materias = await r.json();
    if (!materias.length) { alert("No hay materias para exportar."); return; }
    const cols = ["clave_materia","nombre_materia","creditos_totales","horas_teoricas","horas_practicas","no_unidades"];
    const rows = [cols.join(",")];
    materias.forEach(m => rows.push(cols.map(c=>`"${(m[c]??"").toString().replace(/"/g,'""')}"`).join(",")));
    const blob = new Blob([rows.join("\n")], {type:"text/csv;charset=utf-8;"});
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a"); a.href=url; a.download="materias_RCA.csv"; a.click(); URL.revokeObjectURL(url);
    if (typeof toast==="function") toast("CSV exportado correctamente");
  } catch { alert("Error al exportar materias."); }
}

document.addEventListener("click", e => { if (e.target.id==="modalImportMaterias") cerrarModalCSVMaterias(); });
