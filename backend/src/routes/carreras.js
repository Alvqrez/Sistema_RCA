...
router.put("/:id", soloAdmin, (req, res) => {
  ...
  if (!nombre_carrera) return res.status(400).json({ error: "El nombre es requerido" });
  db.query(
...
router.delete("/:id", soloAdmin, (req, res) => {
  db.query("DELETE FROM carrera WHERE id_carrera=?", [req.params.id], (err, r) => {
    if (err) {
...
router.get("/:id", verificarToken, (req, res) => {
  db.query("SELECT * FROM carrera WHERE id_carrera=?", [req.params.id], (err, rows) => {
    if (err) return res.status(500).json({ error: "Error interno del servidor" });
