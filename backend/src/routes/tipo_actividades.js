...
router.post('/', soloAdmin, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res.status(201).json({ success: true, id_tipo: result.insertId, mensaje: 'Tipo registrado' });
    }
  );
});
  ...
  db.query(
    `UPDATE tipo_actividad
        SET nombre      = ?,
            ...
            activo      = ?
      WHERE id_tipo = ?`,
    [nombre.trim(), descripcion ?? null, activo !== undefined ? activo : 1, req.params.id],
    (err, result) => {
      if (err) {
        if (err.code === 'ER_DUP_ENTRY') {
          return res.status(409).json({ error: 'Ya existe un tipo con ese nombre' });
      ...
      if (!result.affectedRows) return res.status(404).json({ error: 'Tipo no encontrado' });
