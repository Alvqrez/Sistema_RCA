...
router.post("/", soloMaestro, (req, res) => {
    ...
    const { clave_materia, numero_empleado, id_periodo, limite_alumnos, horario, aula } = req.body;

    if (!clave_materia || !numero_empleado || !id_periodo) {
        return res.status(400).json({ error: "Clave de materia, número de empleado y periodo son requeridos" });
    }
    ...
    db.query(query, [clave_materia, numero_empleado, id_periodo, limite_alumnos ?? 30, horario ?? null, aula ?? null], (err, result) => {

        if (err) return res.status(500).json({ error: "Error interno del servidor" });

        res.status(201).json({ success: true, mensaje: "Grupo creado", id_grupo: result.insertId });
...
router.put("/:id", soloMaestro, (req, res) => {
    ...
    db.query(query, [limite_alumnos ?? 30, horario ?? null, aula ?? null, estatus ?? "Activo", req.params.id], (err, result) => {
...
router.delete("/:id", soloMaestro, (req, res) => {
    ...
    db.query("DELETE FROM Grupo WHERE id_grupo = ?", [req.params.id], (err, result) => {
