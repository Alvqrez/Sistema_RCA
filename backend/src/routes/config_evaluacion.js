...
router.get("/:id_grupo/:id_unidad", verificarToken, (req, res) => {
  db.query(
    ...
    (err, rows) => {
      ...
      res.json(rows[0] || {
        id_grupo: parseInt(req.params.id_grupo), id_unidad: parseInt(req.params.id_unidad),
        pct_actividades: 60, pct_examen: 30, pct_asistencia: 10, cal_examen: null, nota: null,
      });
    }
  );
});
...
router.post("/", maestroOAdmin, (req, res) => {
  const { id_grupo, id_unidad, pct_actividades, pct_examen, pct_asistencia, nota } = req.body;

  if (!id_grupo || id_unidad == null)
    return res.status(400).json({ error: "id_grupo e id_unidad son requeridos" });

  // Usar 0 como default real — no || que convierte 0 en el default original
  const pA  = isNaN(parseFloat(pct_actividades)) ? 0 : parseFloat(pct_actividades);
  const pE  = isNaN(parseFloat(pct_examen))      ? 0 : parseFloat(pct_examen);
  const pAs = isNaN(parseFloat(pct_asistencia))  ? 0 : parseFloat(pct_asistencia);
  ...
  if (nota) {
    try {
      ...
      const sumaNota = Object.values(fullPcts).reduce((s, v) => s + (parseFloat(v) || 0), 0);
  ...
  db.query(
    ...
    (err) => {
      if (err) return res.status(500).json({ error: "Error al guardar en base de datos", detail: err.message });
