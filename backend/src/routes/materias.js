...
router.get("/:clave", verificarToken, (req, res) => {
    ...
    db.query("SELECT * FROM Materia WHERE clave_materia = ?", [req.params.clave], (err, results) => {

        if (err) return res.status(500).json({ error: "Error interno del servidor" });

        if (results.length === 0) return res.status(404).json({ error: "Materia no encontrada" });
...
// POST — crear materia (solo maestro)
router.post("/", soloMaestro, (req, res) => {
    ...
    db.query(query, [clave_materia, nombre_materia, creditos_totales ?? 0, horas_teoricas ?? 0, horas_practicas ?? 0, no_unidades ?? 0], (err) => {
...
// PUT — editar materia (solo maestro)
router.put("/:clave", soloMaestro, (req, res) => {
    ...
    db.query(query, [nombre_materia, creditos_totales ?? 0, horas_teoricas ?? 0, horas_practicas ?? 0, no_unidades ?? 0, req.params.clave], (err, result) => {
...
// DELETE — eliminar materia (solo maestro)
router.delete("/:clave", soloMaestro, (req, res) => {
