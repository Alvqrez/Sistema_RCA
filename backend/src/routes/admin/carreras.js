...
// GET — todas las carreras (campos completos)
router.get("/", verificarToken, (req, res) => {
  db.query(
    `SELECT id_carrera, nombre_carrera, siglas, plan_estudios,
            total_semestres, total_creditos
...
router.post("/", soloAdmin, (req, res) => {
  const { id_carrera, nombre_carrera, siglas, plan_estudios, total_semestres, total_creditos } = req.body;
  ...
  db.query(
    `INSERT INTO carrera (id_carrera, nombre_carrera, siglas, plan_estudios, total_semestres, total_creditos)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [id_carrera, nombre_carrera, siglas ?? null, plan_estudios ?? null, total_semestres ?? null, total_creditos ?? null],
    (err) => {
      if (err) {
...
router.put("/:id", soloAdmin, (req, res) => {
  const { nombre_carrera, siglas, plan_estudios, total_semestres, total_creditos } = req.body;
  if (!nombre_carrera)
  ...
  db.query(
    `UPDATE carrera
     ...
     WHERE id_carrera=?`,
    [nombre_carrera, siglas ?? null, plan_estudios ?? null, total_semestres ?? null, total_creditos ?? null, req.params.id],
    (err, r) => {
      if (err) return res.status(500).json({ error: "Error interno del servidor" });
      if (!r.affectedRows) return res.status(404).json({ error: "Carrera no encontrada" });
    ...
    (err, r) => {
      if (err) {
        if (err.code === "ER_ROW_IS_REFERENCED_2")
          return res.status(409).json({ error: "No se puede eliminar: hay alumnos o retículas vinculadas" });
        return res.status(500).json({ error: "Error interno del servidor" });
      }
      if (!r.affectedRows) return res.status(404).json({ error: "Carrera no encontrada" });
