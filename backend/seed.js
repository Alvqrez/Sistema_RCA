...
async function seed() {
  try {
    ...
    // 2. CARRERAS
    // ══════════════════════════════════════════════════════════════════════
    await q(`INSERT IGNORE INTO carrera (id_carrera, nombre_carrera, siglas) VALUES
    ...
    await q(`INSERT IGNORE INTO materia (clave_materia, nombre_materia, no_unidades) VALUES
               ...
               ('APM001','Aprendizaje Automático',3),
               ('VIS001','Visión por Computadora',2),
               ('NLP001','Procesamiento de Lenguaje Natural',3),
               ('EST001','Estadística para IA',4),
               ('ADM001','Administración de Empresas',3),
               ('FIN001','Finanzas Empresariales',4),
               ('MKT001','Mercadotecnia',2),
               ('LOG001','Logística y Cadena de Suministro',3),
               ('MAT001','Matemáticas Aplicadas',5),
               ('ETI001','Ética Profesional',2),
               ('ING001','Inglés Técnico',2)`);
    ...
    await q(`INSERT IGNORE INTO reticula (clave_materia, id_carrera, semestre) VALUES
               ...
               ('RED001','ISC',6), ('MAT001','ISC',1),
               ('APM001','IIA',4), ('VIS001','IIA',5), ('NLP001','IIA',6),
               ('EST001','IIA',3), ('MAT001','IIA',1),
               ('ADM001','IGE',2), ('FIN001','IGE',4), ('MKT001','IGE',5),
               ('LOG001','IGE',6), ('ETI001','IGE',8)`);
    ...
    for (const a of alumnos) {
      await q(
        `INSERT IGNORE INTO alumno
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [a.nc, a.car, a.nom, a.ap, a.am, a.curp, a.fn, a.ci, a.cp, a.cel],
      );
      await q(
