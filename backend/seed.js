...
async function seed() {
  try {
    ...
    // ── 1. Administrador ──────────────────────────────────────────────────
    await query(`INSERT IGNORE INTO administrador (nombre, apellido_paterno, correo_institucional)
                     VALUES ('Admin', 'Sistema', 'admin@itver.edu.mx')`);

    const admins = await query(
      "SELECT id_admin FROM administrador WHERE correo_institucional = 'admin@itver.edu.mx'",
    );
