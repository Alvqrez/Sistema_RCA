...
async function seed() {
  try {
    ...
    // ── 2 Maestros ────────────────────────────────────────────────────────
    await q(`INSERT IGNORE INTO maestro (numero_empleado,nombre,apellido_paterno,apellido_materno,curp,correo_institucional,departamento)
             VALUES ('EMP001','Juan','Pérez','López','PELJ800101HVZRPN01','jperez@itver.edu.mx','Sistemas Computacionales')`);
    await q(`INSERT IGNORE INTO maestro (numero_empleado,nombre,apellido_paterno,apellido_materno,curp,correo_institucional,departamento)
             VALUES ('EMP002','María','García','Soto','GASM850215MVZRTN02','mgarcia@itver.edu.mx','Sistemas Computacionales')`);

    const hM1 = await bcrypt.hash("maestro123", 10);
    const hM2 = await bcrypt.hash("maestro456", 10);
    await q(
      `INSERT IGNORE INTO usuario (username,pwd,rol,id_referencia) VALUES (?,?,'maestro','EMP001')`,
      ["profe01", hM1],
    );
    await q(
      `INSERT IGNORE INTO usuario (username,pwd,rol,id_referencia) VALUES (?,?,'maestro','EMP002')`,
      ["profe02", hM2],
    );
    console.log("✓ Maestro1 →  profe01 / maestro123");
    console.log("✓ Maestro2 →  profe02 / maestro456");
    ...
    // ── Retícula ──────────────────────────────────────────────────────────
    await q(
      `INSERT IGNORE INTO reticula (clave_materia,id_carrera,semestre,creditos) VALUES ('FBD001','ISC',4,5)`,
    );
    await q(
      `INSERT IGNORE INTO reticula (clave_materia,id_carrera,semestre,creditos) VALUES ('POO001','ISC',3,5)`,
    );

    // ── Unidades ─────────────────────────────────────────────────────────
    // FBD001 → 3 unidades
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (1,'FBD001','Modelo Entidad-Relación','Cerrada')`,
    );
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (2,'FBD001','Modelo Relacional','Cerrada')`,
    );
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (3,'FBD001','SQL y Consultas','En curso')`,
    );
    // POO001 → 3 unidades
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (4,'POO001','Clases y Objetos','Cerrada')`,
    );
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (5,'POO001','Herencia y Polimorfismo','En curso')`,
    );
    await q(
      `INSERT IGNORE INTO unidad (id_unidad,clave_materia,nombre_unidad,estatus) VALUES (6,'POO001','Patrones de Diseño','Pendiente')`,
    );
    console.log("✓ Unidades →  6 unidades (3 por materia)");

    // ── 4 Grupos (2 por materia) ──────────────────────────────────────────
    // FBD Grupo A (EMP001), FBD Grupo B (EMP002)
    await q(`INSERT IGNORE INTO grupo (id_grupo,clave_materia,numero_empleado,id_periodo,limite_alumnos,aula,horario)
             VALUES (1,'FBD001','EMP001',1,35,'Aula 101','Lun-Mié-Vie 07:00-08:00')`);
    await q(`INSERT IGNORE INTO grupo (id_grupo,clave_materia,numero_empleado,id_periodo,limite_alumnos,aula,horario)
             VALUES (2,'FBD001','EMP002',1,35,'Aula 102','Mar-Jue 10:00-12:00')`);
    // POO Grupo A (EMP001), POO Grupo B (EMP002)
    await q(`INSERT IGNORE INTO grupo (id_grupo,clave_materia,numero_empleado,id_periodo,limite_alumnos,aula,horario)
             VALUES (3,'POO001','EMP001',1,30,'Lab 201','Lun-Mié 09:00-11:00')`);
    await q(`INSERT IGNORE INTO grupo (id_grupo,clave_materia,numero_empleado,id_periodo,limite_alumnos,aula,horario)
             VALUES (4,'POO001','EMP002',1,30,'Lab 202','Mar-Jue-Vie 13:00-14:00')`);
    ...
    // ── Inscripciones ─────────────────────────────────────────────────────
    // Todos en Grupo 1 FBD, 2023001 y 2023002 también en Grupo 3 POO
    const inscripciones = [
      ["2023001", 1],
      ["2023002", 1],
      ["2023003", 1],
    ...
    // Alumno 2023001 — desempeño variable
    const cals = [
      // [matricula, id_actividad, calificacion, estatus, empleado]
      ["2023001", 1, 85, "Validada", "EMP001"],
      ["2023001", 2, 72, "Validada", "EMP001"],
      ["2023001", 3, 90, "Validada", "EMP001"],
      ["2023002", 1, 60, "Validada", "EMP001"],
      ["2023002", 2, 55, "Validada", "EMP001"],
      ["2023002", 3, 70, "Validada", "EMP001"],
      ["2023003", 1, 95, "Validada", "EMP001"],
      ["2023003", 2, 88, "Validada", "EMP001"],
      ["2023003", 3, 80, "Validada", "EMP001"],
      ["2023004", 1, 45, "Validada", "EMP001"],
      ["2023004", 2, 50, "Validada", "EMP001"],
      ["2023004", 3, 60, "Validada", "EMP001"],
      // Unidad 2
      ["2023001", 6, 78, "Validada", "EMP001"],
      ["2023001", 7, 81, "Validada", "EMP001"],
      ["2023002", 6, 65, "Validada", "EMP001"],
      ["2023002", 7, 68, "Validada", "EMP001"],
      ["2023003", 6, 92, "Validada", "EMP001"],
      ["2023003", 7, 95, "Validada", "EMP001"],
      ["2023004", 6, 50, "Validada", "EMP001"],
      ["2023004", 7, 55, "Validada", "EMP001"],
    ];
    for (const [mat, act, cal, est, emp] of cals) {
      await q(
        `INSERT IGNORE INTO resultado_actividad (matricula,id_actividad,calificacion_obtenida,estatus,numero_empleado)
               VALUES (?,?,?,?,?)`,
        [mat, act, cal, est, emp],
      );
    }
    ...
    // Unidad 1: Cal = Tarea(20%)+Examen(60%)+Parti(20%)
    const cuData = [
    ...
    await q(`INSERT IGNORE INTO bonusunidad (matricula,id_unidad,id_grupo,numero_empleado,puntos_otorgados,justificacion,fecha_asignacion,estatus)
             VALUES ('2023003',2,1,'EMP001',3.00,'Excelente participación en proyectos','2025-03-15','Activo')`);
    await q(`UPDATE calificacion_unidad SET calificacion_unidad_final = LEAST(100, calificacion_unidad_final + 3), estatus_unidad = 'Aprobada'
    ...
    console.log("  profe01  / maestro123  (EMP001)");
    console.log("  profe02  / maestro456  (EMP002)");
